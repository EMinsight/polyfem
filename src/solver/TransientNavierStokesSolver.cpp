...
namespace polyfem
{
		...
		Eigen::VectorXd &x)
	{
		...
		time.start();
		assembler.assemble_energy_hessian(state.formulation() + "Picard", state.mesh->is_volume(), state.n_bases, false, state.bases, gbases, x, state.index_mapping, nl_matrix);
		AssemblerUtils::merge_mixed_matrices(state.n_bases, state.n_pressure_bases, problem_dim, state.use_avg_pressure,
		...
		while (nlres_norm > grad_norm && it < iterations)
		{
			...
			{
				assembler.assemble_energy_hessian(formulation, state.mesh->is_volume(), state.n_bases, false, state.bases, gbases, x, state.index_mapping, nl_matrix);
				AssemblerUtils::merge_mixed_matrices(state.n_bases, state.n_pressure_bases, problem_dim, state.use_avg_pressure,
													 (velocity_stiffness + nl_matrix) + velocity_mass, mixed_stiffness, pressure_stiffness,
													 total_matrix);
			}
